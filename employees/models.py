...
class Attendance(models.Model):
    ...
    local_clock_in_time = models.DateTimeField(
        null=True, blank=True, help_text="Clock-in time in employee's local timezone"
    )
    local_clock_out_time = models.DateTimeField(
        null=True, blank=True, help_text="Clock-out time in employee's local timezone"
    )
    ...
    def calculate_late_arrival_with_timezone(self, user_timezone_str):
        ...
        grace_period = getattr(
            shift, "grace_period_minutes", 15
        )  # Default 15 minutes grace
    ...
    def should_stop_location_tracking(self):
        ...
        if (
            self.location_tracking_end_time
            and timezone.now() >= self.location_tracking_end_time
        ):
            return True
...
class LeaveBalance(models.Model):
    ...
    def total_balance(self):
        return (
            self.casual_leave_balance + self.sick_leave_balance + self.comp_off_balance
        )
