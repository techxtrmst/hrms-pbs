    ...
    def handle(self, *args, **options):
        ...
        for balance in leave_balances:
            ...
            total_cl_approved = sum(
                leave.total_days for leave in approved_leaves if leave.leave_type == 'CL'
            )
            total_sl_approved = sum(
                leave.total_days for leave in approved_leaves if leave.leave_type == 'SL'
            )
            total_el_approved = sum(
                leave.total_days for leave in approved_leaves if leave.leave_type == 'EL'
            )
            total_co_approved = sum(
                leave.total_days for leave in approved_leaves if leave.leave_type == 'CO'
            )
            total_ul_approved = sum(
                leave.total_days for leave in approved_leaves if leave.leave_type == 'UL'
            )
            ...
            needs_fix = (
                balance.casual_leave_used != cl_should_use or
                balance.sick_leave_used != sl_should_use or
                balance.earned_leave_used != el_should_use or
                balance.comp_off_used != co_should_use or
        ...
        if fixed_count == 0:
            ...
            if dry_run:
                self.stdout.write(
                    self.style.WARNING(f'Would fix {fixed_count} employees with incorrect balances')
                )
            else:
                self.stdout.write(
                    self.style.SUCCESS(f'Fixed incorrect balances for {fixed_count} employees')
                )