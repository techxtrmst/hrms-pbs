...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for balance in leave_balances:
            ...
            approved_leaves = LeaveRequest.objects.filter(employee=balance.employee, status="APPROVED")

            # Calculate totals by leave type
            total_cl_approved = sum(leave.total_days for leave in approved_leaves if leave.leave_type == "CL")
            total_sl_approved = sum(leave.total_days for leave in approved_leaves if leave.leave_type == "SL")
            total_el_approved = sum(leave.total_days for leave in approved_leaves if leave.leave_type == "EL")
            total_co_approved = sum(leave.total_days for leave in approved_leaves if leave.leave_type == "CO")
            total_ul_approved = sum(leave.total_days for leave in approved_leaves if leave.leave_type == "UL")
            ...
            total_lop_should_be = cl_excess + sl_excess + el_excess + co_excess + total_ul_approved
            ...
            if needs_fix:
                ...
                self.stdout.write(f"  CL: Used {balance.casual_leave_used} → Should be {cl_should_use}")
                self.stdout.write(f"  SL: Used {balance.sick_leave_used} → Should be {sl_should_use}")
                self.stdout.write(f"  EL: Used {balance.earned_leave_used} → Should be {el_should_use}")
                self.stdout.write(f"  CO: Used {balance.comp_off_used} → Should be {co_should_use}")
                self.stdout.write(f"  LOP: {balance.unpaid_leave} → Should be {total_lop_should_be}")
                ...
                if not dry_run:
                    ...
                    self.stdout.write(self.style.SUCCESS(f"Fixed balances for {employee_name}"))
                else:
                    self.stdout.write(self.style.WARNING(f"Would fix balances for {employee_name}"))
        ...
        if fixed_count == 0:
            ...
            if dry_run:
                self.stdout.write(self.style.WARNING(f"Would fix {fixed_count} employees with incorrect balances"))
            else:
                self.stdout.write(self.style.SUCCESS(f"Fixed incorrect balances for {fixed_count} employees"))
