...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--daemon", action="store_true", help="Run as a daemon (continuous loop)")
        parser.add_argument(
            "--max-age",
            type=int,
    ...
    def process_location_tracking(self, max_age_seconds):
        ...
        clocked_in_attendances = Attendance.objects.filter(date=today, is_currently_clocked_in=True).select_related(
    ...
    def should_track_employee(self, employee, attendance, now, max_age_seconds):
        ...
        last_log = LocationLog.objects.filter(employee=employee, log_type="HOURLY").order_by("-timestamp").first()
    ...
    def cleanup_old_logs(self, days=30):
        ...
        deleted_count = LocationLog.objects.filter(
            timestamp__lt=cutoff_date, log_type="SYSTEM_REQUEST", is_valid=False
        ).delete()[0]
