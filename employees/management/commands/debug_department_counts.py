...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--company-id", type=int, help="Filter by specific company ID"
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS("  DEPARTMENT EMPLOYEE COUNT DEBUG REPORT")
        )
        self.stdout.write(self.style.SUCCESS("=" * 80))
        ...
        null_dept_count = employees.filter(
            Q(department__isnull=True) | Q(department="")
        ).count()
        if null_dept_count > 0:
            issues_found = True
            self.stdout.write(
                f"⚠️  {null_dept_count} employees have NULL/empty department"
            )
        ...
        for dept in departments_list:
            ...
            percentage = round(
                (dept_present / dept_total * 100) if dept_total > 0 else 0, 1
            )
        ...
        if total_shown != employees.count():
            self.stdout.write(
                self.style.ERROR(
                    f"⚠️  MISMATCH! Dashboard shows {total_shown} but DB has {employees.count()}"
                )
            )
        else:
        ...
        if fix_data and issues_found:
            ...
            for emp in employees:
                if emp.department and emp.department.strip() != emp.department:
                    ...
                    fixed_count += 1
                    self.stdout.write(
                        f"  Fixed: '{old_dept}' -> '{emp.department}' for {emp.user.get_full_name()}"
                    )

            if fixed_count > 0:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✅ Fixed {fixed_count} department whitespace issues"
                    )
                )
            else:
        ...
        if null_dept_count > 0:
            self.stdout.write(
                "📝 Assign departments to employees with NULL/empty department"
            )
        ...
        self.stdout.write(
            "📝 Ensure all employees have is_active=True if they should appear in dashboard"
        )
