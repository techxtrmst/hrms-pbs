...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--company-id", type=int, help="Filter by specific company ID")
    ...
    def handle(self, *args, **options):
        ...
        if whitespace_issues:
            issues_found = True
            self.stdout.write("⚠️  Departments with whitespace issues:")
            for dept in whitespace_issues:
                self.stdout.write(f"     '{dept}' -> '{dept.strip()}'")

        # Check for similar department names
        [d.strip().lower() for d in dept_counts if d and d.strip()]
        similar_depts = {}
        for dept in dept_counts:
        ...
        for normalized, variations in similar_depts.items():
            if len(variations) > 1:
                ...
                if not issues_found:
                    issues_found = True
                self.stdout.write("⚠️  Similar department names found:")
        ...
        if total_shown != employees.count():
            self.stdout.write(
                self.style.ERROR(f"⚠️  MISMATCH! Dashboard shows {total_shown} but DB has {employees.count()}")
            )
        else:
