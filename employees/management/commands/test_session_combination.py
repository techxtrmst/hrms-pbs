...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--employee-id',
            type=int,
            help='Test with specific employee ID'
        )
        parser.add_argument(
            '--create-test-data',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if shift:
            ...
            self.stdout.write(f'⏰ Shift: {shift.name} ({shift.start_time} - {shift.end_time}) = {expected_hours:.1f} hours')
        else:
            expected_hours = 9.0
            self.stdout.write(f'⏰ Default shift: 9.0 hours')
        ...
        self.stdout.write(f'   ')
        self.stdout.write(f'   📊 WORKING HOURS CALCULATION:')
        ...
        if sessions.exists():
            ...
            self.stdout.write(f'   ')
            self.stdout.write(f'   🧮 COMBINATION VERIFICATION:')
            ...
            self.stdout.write(f'   Match: {"✅ Correct" if abs(total_combined - summary["total_worked_hours"]) < 0.01 else "❌ Error"}')
        ...
        if summary["active_sessions"] > 0:
            self.stdout.write(f'   • ⚠️  Employee currently clocked in - session incomplete')
        
        if summary["total_sessions"] > 3:
            self.stdout.write(f'   • ℹ️  Multiple sessions detected - all properly combined')
        
        self.stdout.write(f'\n✅ Session combination test completed!')
        
        # Usage examples
        self.stdout.write(f'\n📚 USAGE EXAMPLES:')
        self.stdout.write(f'   # Test with specific employee')
        self.stdout.write(f'   python manage.py test_session_combination --employee-id 1')
        self.stdout.write(f'   ')
        self.stdout.write(f'   # Create test data and analyze')
        self.stdout.write(f'   python manage.py test_session_combination --create-test-data')
        self.stdout.write(f'   ')
        self.stdout.write(f'   # Test with specific employee and create data')
        self.stdout.write(f'   python manage.py test_session_combination --employee-id 1 --create-test-data')