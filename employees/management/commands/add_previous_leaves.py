...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--employee-id',
            type=int,
            help='Add leaves for specific employee ID only'
        )
        parser.add_argument(
            '--company-id',
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        if not employee_id and not company_id:
            self.stdout.write(
                self.style.ERROR('❌ Please specify either --employee-id or --company-id')
            )
            return
        ...
        if employee_id:
            try:
                ...
                if not employees.exists():
                    self.stdout.write(
                        self.style.ERROR(f'❌ Employee with ID {employee_id} not found or inactive')
                    )
                    return
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f'❌ Error finding employee: {e}')
                )
                return
        elif company_id:
            try:
                ...
                if not employees.exists():
                    self.stdout.write(
                        self.style.ERROR(f'❌ No active employees found for company {company.name}')
                    )
                    return
        ...
        self.stdout.write(f'📊 Leaves to add:')
        if casual_leave > 0:
        ...
        if dry_run or total_updated > 0:
            self.stdout.write(f'\n📚 USAGE EXAMPLES:')
            self.stdout.write(f'   # Add 5 CL and 3 SL to specific employee')
            self.stdout.write(f'   python manage.py add_previous_leaves --employee-id 1 --casual-leave 5 --sick-leave 3')
            self.stdout.write(f'   ')
            self.stdout.write(f'   # Add 2.5 CL to all employees in company')
            self.stdout.write(f'   python manage.py add_previous_leaves --company-id 1 --casual-leave 2.5')
            self.stdout.write(f'   ')
            self.stdout.write(f'   # Test before applying (dry run)')
            self.stdout.write(f'   python manage.py add_previous_leaves --employee-id 1 --casual-leave 5 --dry-run')
        
        # Show integration note
        if not dry_run and total_updated > 0:
            self.stdout.write(f'\n✅ INTEGRATION NOTE:')
            self.stdout.write(f'   • These manual adjustments will be preserved during monthly accrual')
            self.stdout.write(f'   • Monthly accrual will ADD to these balances, not overwrite them')
            self.stdout.write(f'   • Changes are tracked in carry_forward_leave field for audit')