...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if recalculate_all:
            ...
            self.stdout.write(f'📅 Processing all attendance records')
        else:
        ...
        for attendance in attendances:
            ...
            new_effective_hours = f"{hours}:{minutes:02d}{'+' if has_active_session else ''}" if new_total_hours > 0 else "0:00"
        ...
        if fixed_count > 0:
            self.stdout.write(f'\n📋 SUMMARY OF CHANGES:')
            self.stdout.write(f'   • Only completed sessions counted (clock-in + clock-out)')
            self.stdout.write(f'   • Incomplete sessions ignored (until regularization)')
            self.stdout.write(f'   • Realistic daily hours (typically 6-10 hours)')
            self.stdout.write(f'   • Multiple sessions properly summed')
        
        if problematic_count > 0:
            self.stdout.write(f'\n⚠️  PROBLEMATIC RECORDS:')
            self.stdout.write(f'   • {problematic_count} records with >16 hours/day')
            self.stdout.write(f'   • These may need manual review')
            self.stdout.write(f'   • Check for missing clock-outs or data issues')