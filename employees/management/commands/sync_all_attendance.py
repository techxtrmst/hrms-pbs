...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--start-year", type=int, help="Start year for multi-year sync (optional)")
        parser.add_argument("--end-year", type=int, help="End year for multi-year sync (optional)")
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("  ATTENDANCE SYNC - Complete Attendance Record Synchronization"))
        self.stdout.write(self.style.SUCCESS("=" * 70))

        # Determine which years to process
        years_to_process = range(start_year, end_year + 1) if start_year and end_year else [year]
        ...
        for process_year in years_to_process:
            ...
            try:
                if month:
                    call_command("sync_leave_holiday_attendance", year=process_year, month=month)
                else:
        ...
        # Final summary
        self.stdout.write(f"\n{'=' * 70}")
        self.stdout.write(self.style.SUCCESS("✓ ATTENDANCE SYNC COMPLETED SUCCESSFULLY"))
        self.stdout.write(self.style.SUCCESS("=" * 70))
