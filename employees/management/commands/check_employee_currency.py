...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for loc in Location.objects.all():
            self.stdout.write(
                f"{loc.name} - {loc.country_code} - Currency: {loc.currency} - Company: {loc.company.name}"
            )
        ...
        for emp in employees:
            ...
            self.stdout.write(
                f"{emp.user.get_full_name()} - Location: {location_info} - Currency: {currency_info} - Country: {country_info} - Company: {emp.company.name if emp.company else 'None'}"
            )
