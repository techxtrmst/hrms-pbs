...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            for employee in employees:
                try:
                    ...
                    if created:
                        self.stdout.write(
                            self.style.WARNING(f"Created missing leave balance for {employee.user.get_full_name()}")
                        )
                        issues_found += 1
                        if fix_issues:
                            issues_fixed += 1
                    ...
                    if has_negative and fix_issues:
                        ...
                        issues_fixed += 1
                        self.stdout.write(
                            self.style.SUCCESS(f"Fixed negative values for {employee.user.get_full_name()}")
                        )
                    ...
                    self.stdout.write(self.style.ERROR(f"Error processing {employee.user.get_full_name()}: {str(e)}"))
                    issues_found += 1
        ...
        if issues_found > 0:
            ...
            self.stdout.write(self.style.SUCCESS("No issues found! All leave balances are consistent."))
