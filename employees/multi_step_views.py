...
def add_employee_step1(request):
    ...
    return render(
        request, "employees/add_employee_step1.html", {"form": form, "step": 1}
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            request.session["employee_job_data"] = {
                ...
                "manager_id": form.cleaned_data.get("manager").id
...
def add_employee_step3(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"Employee {employee.user.get_full_name()} created successfully! Password: {password}",
            )
