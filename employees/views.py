...
from .models import Employee, Attendance, LocationLog, LeaveRequest, LeaveBalance, RegularizationRequest
from core.email_utils import send_leave_request_notification
from .forms import EmployeeCreationForm, LeaveApplicationForm, EmployeeUpdateForm, EmployeeBulkImportForm, RegularizationRequestForm
        ...
        try:
            ...
            try:
                ...
                if attendance.clock_in:
                    ...
                    logger.info(f"Duplicate clock-in attempt #{attendance.clock_in_attempts} by {employee.user.username} on {today}")
            ...
            if status == 'WFH':
                try:
                    ...
                    recipient_list = [admin.email for admin in User.objects.filter(role=User.Role.COMPANY_ADMIN) if admin.email]
            ...
            response_data = {
                ...
                'tracking_end_time': attendance.location_tracking_end_time.strftime('%H:%M:%S')
            }
            
            if attendance.is_late:
                response_data['is_late'] = True
                response_data['late_by_minutes'] = attendance.late_by_minutes
                response_data['message'] = f'Clocked in successfully. You are {attendance.late_by_minutes} minutes late.'
            else:
        ...
        try:
            ...
            try:
                ...
                if not force_clockout and not attendance.is_shift_complete():
                    ...
                    if attendance.clock_in:
                        worked_hours = (timezone.now() - attendance.clock_in).total_seconds() / 3600
                ...
                if attendance.is_early_departure:
                    ...
                    response_data['early_by_minutes'] = attendance.early_departure_minutes
                    response_data['message'] = f'Clocked out. You left {attendance.early_departure_minutes} minutes early.'
        ...
        if lat is not None and lng is not None:
            ...
            try:
                ...
                if attendance.location_tracking_active:
                    LocationLog.objects.create(employee=employee, latitude=lat, longitude=lng)
                    return JsonResponse({
                        'status': 'success', 
...
def approve_leave(request, pk):
    if request.method == 'POST':
        ...
        is_manager = user.role == User.Role.MANAGER and leave_request.employee.manager == user
...
def attendance_map(request, pk):
    try:
        ...
        return render(request, 'core/attendance_map.html', {
    ...
    try:
        ...
        if map_attendance:
            ...
            if map_attendance.location_in:
                ...
                if lat:
                    map_data.append({'lat': lat, 'lng': lng, 'title': f"Clock In ({map_attendance.clock_in.strftime('%H:%M')})", 'type': 'start'})
            ...
            if map_attendance.clock_in:
                ...
                for log in logs:
                    map_data.append({'lat': float(log.latitude), 'lng': float(log.longitude), 'title': f"Log: {log.timestamp.strftime('%H:%M')}", 'type': 'log'})
                    
            # Clock Out Marker
            if map_attendance.location_out:
                lat, lng = parse_loc(map_attendance.location_out)
                if lat:
                    map_data.append({'lat': lat, 'lng': lng, 'title': f"Clock Out ({map_attendance.clock_out.strftime('%H:%M')})", 'type': 'end'})
...
def employee_exit_action(request, pk):
    ...
    try:
        ...
        if not exit_type or exit_type not in ['RESIGNATION', 'ABSCONDED', 'TERMINATED']:
            return JsonResponse({
                'status': 'error',
                'message': 'Invalid exit type selected.'
            }, status=400)
        ...
        if not exit_note:
            return JsonResponse({
                'status': 'error',
                'message': 'Exit reason/note is required.'
            }, status=400)
        ...
        if exit_type == 'RESIGNATION':
            ...
            messages.success(request, f'Resignation request from {employee.user.get_full_name()} has been submitted for approval.')
...
class BulkEmployeeImportView(LoginRequiredMixin, CompanyAdminRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            required_columns = ['first_name', 'last_name', 'email', 'designation', 'department', 'date_of_joining', 'date_of_birth']
            ...
            for index, row in df.iterrows():
                try:
                    with transaction.atomic():
                        ...
                        location = None
                        if location_name:
                            location = Location.objects.filter(company=self.request.user.company, name__iexact=location_name).first()
                        ...
                        if badge_id == 'nan': badge_id = None
                        elif badge_id.endswith('.0'): badge_id = badge_id[:-2] # 101.0 -> 101
            ...
            if success_count > 0:
                messages.success(self.request, f"Successfully created {success_count} employees. Activation emails sent.")
...
def download_sample_import_file(request):
    ...
    # Define columns
    columns = ['First Name', 'Last Name', 'Email', 'Designation', 'Department', 'Mobile', 
               'Date of Joining', 'Date of Birth', 'Badge ID', 'Gender', 'Marital Status', 'Annual CTC', 'Location']
    ...
    try:
        ...
        return HttpResponse("Error: openpyxl library not found. Please contact admin to install it.", status=500)
    except Exception as e:
...
def approve_regularization(request, pk):
    if request.method == 'POST':
        ...
        user = request.user
        is_admin = user.role == User.Role.COMPANY_ADMIN
        is_manager = user.role == User.Role.MANAGER and reg_request.employee.manager == user
        ...
        if reg_request.check_out:
            attendance.clock_out = timezone.make_aware(timezone.datetime.combine(reg_request.date, reg_request.check_out))
...
def reject_regularization(request, pk):
    if request.method == 'POST':
        ...
        reg_request.manager_comment = request.POST.get('rejection_reason', '') # Use manager_comment for rejection reason
        reg_request.save()
...
def leave_configuration(request):
    ...
    employees = employees.select_related('leave_balance', 'user').order_by('user__first_name')
    
    return render(request, 'employees/leave_configuration.html', {
        'employees': employees
    })
...
def run_monthly_accrual(request):
    ...
    try:
        ...
        messages.success(request, 'Monthly accrual processed: +1 Sick and +1 Casual leave added to all employees.')
    except Exception as e:
            ...
            if action == 'upload':
                ...
                if 'aadhar_front' in request.FILES:
                    if can_upload(id_proofs.aadhar_front):
                        ...
                        messages.warning(request, 'Aadhar Front already uploaded. Only admin can replace.')
                ...
                if 'aadhar_back' in request.FILES:
                    if can_upload(id_proofs.aadhar_back):
                        ...
                        messages.warning(request, 'Aadhar Back already uploaded. Only admin can replace.')
                ...
                if 'pan_card' in request.FILES:
                    if can_upload(id_proofs.pan_card):
                        ...
                        messages.warning(request, 'PAN Card already uploaded. Only admin can replace.')
