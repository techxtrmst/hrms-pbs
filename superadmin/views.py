...
def superadmin_dashboard(request, selected_company=None, selected_company_id=None):
    ...
    company_overview = Company.objects.filter(is_active=True).annotate(
...
def export_data_view(request, report_type):
    ...
    if report_type == 'employees':
        # Export employees
        writer.writerow(['Name', 'Email', 'Company', 'Department', 'Designation', 'Join Date', 'Status'])
        ...
        # Export leaves
        writer.writerow(['Employee', 'Company', 'Leave Type', 'Start Date', 'End Date', 'Days', 'Status'])
