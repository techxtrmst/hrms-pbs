...
def get_employee_lifecycle_data(company_id):
    ...
    ).select_related('user').prefetch_related('attendances', 'leave_requests')
    ...
    for emp in employees:
        ...
        leaves_taken = emp.leave_requests.filter(
            status='APPROVED'
        ).count()
...
def get_leave_analytics(company_id, months=6):
    ...
    leave_distribution = LeaveRequest.objects.filter(
    ...
    # Monthly trends
    monthly_trends = LeaveRequest.objects.filter(
        employee__company_id=company_id,
        status='APPROVED',
        start_date__gte=start_date
    ).annotate(
    ...
    frequent_takers = Employee.objects.filter(
        company_id=company_id
    ).annotate(
        leave_count=Count('leave_requests', filter=Q(
...
def get_employee_recent_activity(employee, limit=10):
    ...
    for att in recent_attendance:
        ...
        if att.clock_out:
            ...
            activities.append({
                'type': 'clock_out',
                'timestamp': att.clock_out if timezone.is_aware(att.clock_out) else timezone.make_aware(att.clock_out),
