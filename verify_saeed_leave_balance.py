...
def main():
    ...
    try:
        ...
        approved_requests = LeaveRequest.objects.filter(
            employee=employee, status="APPROVED", start_date__year=current_year
        ).order_by("start_date")
        ...
        if approved_requests.exists():
            ...
            print("\n--- Verification ---")
            print(
                f"CL Balance should be: {balance.casual_leave_allocated} - {total_cl} = {balance.casual_leave_allocated - total_cl}"
            )
            print(f"CL Balance actually is: {balance.casual_leave_balance}")
            print(
                f"SL Balance should be: {balance.sick_leave_allocated} - {total_sl} = {balance.sick_leave_allocated - total_sl}"
            )
            print(f"SL Balance actually is: {balance.sick_leave_balance}")

            if (
                balance.casual_leave_allocated - total_cl == balance.casual_leave_balance
                and balance.sick_leave_allocated - total_sl == balance.sick_leave_balance
            ):
