...
from .ai_utils import AttritionPredictor, HRChatbot, ResumeParser
...
def resume_parser_view(request):
    ...
    if request.method == 'POST' and request.FILES.get('resume'):
        ...
        try:
            ...
            duplicate = ResumeParsingJob.objects.filter(
                duplicate_check_hash=job.duplicate_check_hash,
                status='PROCESSED'
            ...
            if job.parsed_email or job.parsed_phone:
                email_duplicate = ResumeParsingJob.objects.filter(
                    parsed_email=job.parsed_email,
                    status='PROCESSED'
                ).exclude(id=job.id).first() if job.parsed_email else None
                
                phone_duplicate = ResumeParsingJob.objects.filter(
                    parsed_phone=job.parsed_phone,
                    status='PROCESSED'
                ).exclude(id=job.id).first() if job.parsed_phone else None
...
def chatbot_query_floating(request):
    ...
    try:
        ...
        if 'could not' in response_text.lower() or 'unable to' in response_text.lower() or 'sorry' in response_text.lower():
            show_escalation = True
