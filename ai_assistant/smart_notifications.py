...
class SmartNotifications:
    ...
    def check_missed_clock_out(employee):
        ...
        try:
            ...
            if attendance.clock_in and not attendance.clock_out:
                ...
                hours_since_clock_in = (timezone.now() - clock_in_time).total_seconds() / 3600
    ...
    def check_pending_approvals(manager):
        ...
        try:
            ...
            team_members = Employee.objects.filter(manager=manager.employee_profile, is_active=True)

            pending_leaves = LeaveRequest.objects.filter(employee__in=team_members, status="PENDING").order_by(
                "created_at"
            )

            if pending_leaves.exists():
                # Check for old pending requests (>2 days)
                old_pending = pending_leaves.filter(created_at__lt=timezone.now() - timedelta(days=2))
        ...
        try:
            ...
            if warnings:
                return {
                    "alert": True,
                    "type": "LOP_THRESHOLD",
                    "severity": "HIGH" if any(w["type"] == "NEGATIVE_BALANCE" for w in warnings) else "MEDIUM",
                    "warnings": warnings,
                }
        except:
            pass
    ...
    def check_contract_expiry(employee, days_ahead=30):
        ...
        if alerts:
            return {
                ...
                "severity": "HIGH" if any(a["days_remaining"] <= 7 for a in alerts) else "MEDIUM",
                "alerts": alerts,
            }
    ...
    def get_all_alerts_for_manager(manager):
        ...
        try:
            ...
            for member in team_members:
                contract_alert = SmartNotifications.check_contract_expiry(member, days_ahead=15)
