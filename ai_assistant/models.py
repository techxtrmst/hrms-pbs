...
class AttritionRisk(models.Model):
    ...
    employee = models.OneToOneField(Employee, on_delete=models.CASCADE, related_name='attrition_risk')
    risk_score = models.FloatField(default=0.0, help_text="Calculated risk score (0-100)")
...
class ResumeParsingJob(models.Model):
    ...
    parsed_skills = models.TextField(null=True, blank=True)  # Comma-separated for backward compatibility
    parsed_skills_json = models.JSONField(null=True, blank=True)  # {"technical": [], "tools": [], "soft": []}
    ...
    def __str__(self):
        return f"Resume {self.id} - {self.parsed_name or 'Unknown'} - {self.uploaded_at}"
