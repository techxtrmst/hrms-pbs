...
class HRChatbot:
    ...
    def get_response(question, employee, role='EMPLOYEE', request=None):
        ...
        # Greetings
        if any(greeting in question_lower for greeting in ['hello', 'hi', 'hey', 'good morning', 'good afternoon', 'good evening']) and len(question_lower) < 20:
    ...
    def _handle_admin_query(query, employee, user_name, request=None):
        ...
        if ('total' in query or 'list' in query or 'check' in query or 'view' in query or 'show' in query or 'count' in query or 'who' in query) and \
           ('employee' in query or 'staff' in query or 'workforce' in query or 'member' in query or 'memebr' in query or 'people' in query or 'person' in query):
            count = Employee.objects.filter(company=employee.company, is_active=True).count()
            dept_counts = Employee.objects.filter(company=employee.company, is_active=True).values('department').annotate(count=Count('id'))
    ...
    def _handle_manager_query(query, employee, user_name, request=None):
        ...
        if 'total' in query and ('employee' in query or 'staff' in query or 'workforce' in query):
             ...
             team_count = Employee.objects.filter(company=employee.company, manager=employee.user, is_active=True).count()
             return {
        ...
        if 'team' in query and any(w in query for w in ['attendance', 'present', 'absent', 'status']):
            try:
                ...
                if hasattr(employee.user, 'role') and (employee.user.role == 'COMPANY_ADMIN' or employee.user.is_superuser):
                    ...
                    team_members = Employee.objects.filter(company=employee.company, manager=employee.user, is_active=True)
                    context_label = "Team"
        ...
        if 'absent' in query or 'who is not here' in query:
            try:
                ...
                if absent_names:
                    ...
                    return {
                        'answer': f"Everyone is present today! 🎉",
                        'type': 'team_attendance'
                    }
            except Exception as e:
                import traceback
        ...
        if 'late' in query:
            try:
                ...
                if late_entries.exists():
                    entries_str = "\n".join([f"• {a.employee.user.get_full_name()} ({a.clock_in.strftime('%H:%M')})" for a in late_entries])
                    return {
        ...
        if 'leave' in query and any(w in query for w in ['request', 'pending', 'approve', 'approval']):
            try:
                ...
                if hasattr(employee.user, 'role') and (employee.user.role == 'COMPANY_ADMIN' or employee.user.is_superuser):
                    pending_count = LeaveRequest.objects.filter(employee__company=employee.company, status='PENDING').count()
                    context_msg = "in the company"
                else:
                    pending_count = LeaveRequest.objects.filter(employee__manager=employee.user, status='PENDING').count()
                    context_msg = "waiting for your approval"

                if pending_count > 0:
                    # Get top 3 pending requests for Quick Actions
                    if hasattr(employee.user, 'role') and (employee.user.role == 'COMPANY_ADMIN' or employee.user.is_superuser):
                        pending_requests = LeaveRequest.objects.filter(employee__company=employee.company, status='PENDING')[:3]
                    else:
                        pending_requests = LeaveRequest.objects.filter(employee__manager=employee.user, status='PENDING')[:3]
                        
                    request_list = []
                    quick_replies = []
                    
                    for req in pending_requests:
                         request_list.append(f"• **{req.employee.user.get_full_name()}**: {req.leave_type} ({req.start_date.strftime('%d %b')} - {req.end_date.strftime('%d %b')})")
                         quick_replies.append({'label': f"Approve {req.employee.user.first_name}", 'value': f"approve leave {req.id}"})
                         quick_replies.append({'label': f"Reject {req.employee.user.first_name}", 'value': f"reject leave {req.id}"})
        ...
        if 'approve leave' in query or 'reject leave' in query:
            try:
                ...
                if match:
                    ...
                    is_authorized = False
                    if employee.user.role == 'COMPANY_ADMIN' or employee.user.is_superuser:
                        is_authorized = True
                 ...
                 return {
                    'answer': f"I couldn't find that leave request. It might have been deleted. 🤷",
                    'type': 'error'
                }
            except Exception as e:
                print(f"Error processing leave action: {e}")
                return {
                    'answer': f"Something went wrong processing that request. 😔",
                    'type': 'error'
                }
        ...
        if any(w in query for w in ['performance', 'insight', 'analytics', 'risk']):
            try:
                ...
                risk_msg = f"• ⚠️ High Attrition Risk: **{risk_count}** employees" if risk_count > 0 else "• ✅ Attrition Risk: **Low** (Team is stable)"
    ...
    def _handle_employee_query(query, employee, user_name, request=None):
        ...
        manager_keywords = ['team', 'subordinate', 'approval', 'approve', 'reject', 'who is', 'pending', 'insight', 'analytics', 'risk', 'late', 'absent', 'total employee', 'total staff']
        
        if any(w in query for w in manager_keywords):
             # Check if they have subordinates OR are marked as Manager/Admin
             if employee.user.subordinates_user.exists() or employee.user.role in ['MANAGER', 'COMPANY_ADMIN'] or employee.user.is_superuser:
        ...
        if 'leave' in query and ('request' in query or 'pending' in query) and 'apply' not in query:
             if employee.user.subordinates_user.exists() or employee.user.role in ['MANAGER', 'COMPANY_ADMIN']:
                  response = HRChatbot._handle_manager_query(query, employee, user_name, request)
                  if response: return response
        ...
        if any(phrase in query for phrase in ['apply sick leave', 'take sick leave', 'sick leave', 'apply sl']):
            return HRChatbot._handle_leave_application(employee, user_name, 'SL')
        
        if any(phrase in query for phrase in ['apply casual leave', 'take casual leave', 'casual leave', 'apply cl']):
            return HRChatbot._handle_leave_application(employee, user_name, 'CL')
        
        if any(phrase in query for phrase in ['apply earned leave', 'take earned leave', 'earned leave', 'apply el']):
            return HRChatbot._handle_leave_application(employee, user_name, 'EL')
        
        # Regularization Action
        if any(phrase in query for phrase in ['regularize attendance', 'regularization', 'regularize', 'missed punch']):
        ...
        if any(w in query for w in ['leave balance', 'leaves left', 'remaining leave', 'how many leave', 'leave quota', 'check leave']):
            return HRChatbot._get_leave_balance_response(employee, user_name)
        
        # Leave Application (general)
        if any(phrase in query for phrase in ['apply leave', 'apply for leave', 'request leave', 'take leave', 'book leave']):
            return {
        ...
        if any(w in query for w in ['portal', 'login', 'access', 'account', 'username', 'password', 'credentials']):
        ...
        if any(w in query for w in ['attendance', 'present', 'check my attendance', 'attendance status', 'today attendance']):
    ...
    def _get_fallback_response(user_name, role):
        ...
        if role == 'MANAGER': topics += manager_topics
        if role == 'COMPANY_ADMIN': topics += manager_topics + admin_topics
...
class ResumeParser:
    ...
    def _extract_skills(text):
        ...
        skill_keywords = [
            ...
            'Git', 'HTML', 'CSS', 'TypeScript', 'C++', 'C#', 'PHP',
