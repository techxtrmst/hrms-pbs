...
class EnhancedResumeParser:
    ...
    TECHNICAL_SKILLS = [
        ...
        'Tailwind CSS', 'Material UI', 'HTML', 'HTML5', 'CSS', 'CSS3', 'SASS', 'LESS',
        'Webpack', 'Vite', 'Babel',
        
        # Backend Frameworks
        'Django', 'Flask', 'FastAPI', 'Node.js', 'Express.js', 'Spring Boot', 'ASP.NET',
    ...
    TOOLS = [
        'Power BI', 'Tableau', 'Excel', 'Google Analytics', 'Figma', 'Adobe XD', 'Sketch',
        'Photoshop', 'Illustrator', 'InDesign', 'VS Code', 'IntelliJ', 'PyCharm', 'Eclipse',
        'Visual Studio', 'Sublime Text', 'Atom'
    ]
    ...
    def parse_resume(file_path):
        ...
        try:
            ...
            parsed_data = {
                ...
                # Skills (categorized)
                'skills': EnhancedResumeParser._extract_skills_legacy(text),  # Comma-separated for backward compatibility
    ...
    def _extract_name(text):
        ...
        banned = ['resume', 'cv', 'curriculum', 'vitae', 'bio', 'profile', 'summary', 'about', 'contact', 'work', 'experience', 
                 'education', 'skills', 'projects', 'certifications', 'languages', 'hobbies', 'declaration', 
                 'developer', 'engineer', 'analyst', 'manager', 'consultant', 'intern', 'student',
                 'hyderabad', 'bangalore', 'mumbai', 'delhi', 'pune', 'chennai', 'india', 'usa', 'uk', 'dubai',
                 'portfolio', 'website', 'personal', 'link', 'github', 'linkedin', 'gmail', 'email', 'phone', 'address',
                 'uber', 'data', 'analytics', 'analysis'] # Explicitly ban "Uber Data Analytics" components
    ...
    def _extract_location(text):
        ...
        known_places = ['india', 'usa', 'uk', 'canada', 'australia', 'germany', 'france', 'singapore', 'dubai', 'uae',
                        'hyderabad', 'bangalore', 'bengaluru', 'mumbai', 'delhi', 'new delhi', 'pune', 'chennai', 'kolkata', 'gurgaon', 'noida', 
                        'bhubaneswar', 'odisha', 'cuttack', 'berhampur', 'karnataka', 'maharashtra', 'telangana', 'tamil nadu', 'andhra pradesh']
        ...
        for match in matches:
            ...
            # 1. Check Bans
            if any(b in match_lower for b in ban_list): continue
    ...
    def _extract_location(text):
        ...
        ban_list = ['python', 'java', 'script', 'react', 'node', 'aws', 'cloud', 'sql', 'data', 'pandas', 'analysis', 'html', 'css', 'developed', 'managed', 'using', 'tools']
        
        lines = text.split('\n')
        
        # 1. Look for explicit "Location: City"
        for line in lines[:20]: 
            match = re.search(r'(?:Location|Address|City)\s*[:|-]\s*([A-Za-z\s,]+)', line, re.IGNORECASE)
            if match:
        ...
        for match in matches:
            # Check bans
            if any(b in match.lower() for b in ban_list): continue
            if "University" in match or "College" in match: continue
    ...
    def _extract_portfolio(text):
        ...
        false_positives = [
            ...
            'three.js', 'd3.js', 'chart.js', 'moment.js', 'angular.js', 'backbone.js',
            'jquery.js', 'ember.js', 'polymer.js', 'meteor.js', 'gatsby.js',
            'intro.js', 'reveal.js', 'socket.io', 'asp.net', 'vb.net'
        ]
        ...
        for url in matches:
            ...
            if any(x in url_lower for x in ['linkedin', 'github', 'gmail', 'yahoo', 'outlook', 'hotmail', '@']):
                continue
    ...
    def _extract_education(text):
        ...
        safe_patterns = [
            ...
            r'\bB\.?E\.\b', r'\bBachelor of Engineering\b', # B.E. with dot is safe
            r'\bM\.?Tech\b', r'\bMaster of Technology\b',
            r'\bM\.?E\.\b', r'\bMaster of Engineering\b', # M.E. with dot is safe
            r'\bMBA\b', r'\bMaster of Business Administration\b',
            r'\bMCA\b', r'\bMaster of Computer Applications\b', r'\bMaster in Computer Applications\b',
            r'\bBCA\b', r'\bBachelor of Computer Applications\b',
            r'\bB\.?Sc\b', r'\bBachelor of Science\b',
            r'\bM\.?Sc\b', r'\bMaster of Science\b',
            r'\bB\.?Com\b', r'\bBachelor of Commerce\b',
            r'\bM\.?Com\b', r'\bMaster of Commerce\b',
            r'\bPh\.?D\b', r'\bDoctorate\b',
            r'\bDiploma\b', r'\bMatriculation\b', r'\bSSC\b', r'\bHSC\b', r'\b10th\b', r'\b12th\b', r'\bIntermediate\b'
        ]
        ...
        for i, line in enumerate(lines):
            ...
            for pat in safe_patterns:
                ...
                if match:
                    ...
                    if 'b.tech' in lower_deg: degree_found = 'B.Tech'
                    elif 'm.tech' in lower_deg: degree_found = 'M.Tech'
                    elif 'mca' in lower_deg: degree_found = 'MCA'
                    elif 'mba' in lower_deg: degree_found = 'MBA'
                    break
            ...
            if degree_found:
                ...
                if i+1 < len(lines): context_lines.append(lines[i+1].strip())
                if i+2 < len(lines): context_lines.append(lines[i+2].strip())
                
                full_context = ' | '.join([l for l in context_lines if l]) # Use separator
                ...
                if gpa_match:
                    ...
                    if fallback:
                        try: gpa = float(fallback.group(1))
                        except: pass

                # C. Extract University / College
                # Strategy: Look for specific keywords or assume non-degree parts are Uni
                uni_keywords = ['University', 'College', 'Institute', 'School', 'Academy', 'Campus', 'Vidyalaya']
                ...
                specs = ['Computer Science', 'Information Technology', 'Civil', 'Mechanical', 'Electrical', 'Electronics', 'HR', 'Finance', 'Marketing', 'AI', 'Data Science']
                for sp in specs:
    ...
    def _extract_projects(text):
        ...
        for i, line in enumerate(lines):
            ...
            if in_projects_section and any(keyword in line_lower for keyword in ['experience', 'education', 'certifications', 'skills']):
                break
            ...
            if in_projects_section and line.strip() and len(line.strip()) > 5:
                ...
                for j in range(i+1, min(i+15, len(lines))):
                    ...
                    if next_line:
                        ...
                        if len(next_line) < 40 and j > i+2 and not any(kw in next_line.lower() for kw in EnhancedResumeParser.TECHNICAL_SKILLS) and next_line[0].isupper():
    ...
    def _extract_certifications(text):
        ...
        for i, line in enumerate(lines):
            ...
            if in_cert_section and any(keyword in line_lower for keyword in ['experience', 'education', 'projects', 'skills']):
                break
    ...
    def _determine_role_fit(text):
        ...
        role_keywords = {
            ...
            'Backend Developer': ['django', 'flask', 'node.js', 'express', 'spring boot', 'backend', 'api'],
            'Full Stack Developer': ['full stack', 'fullstack', 'mern', 'mean', 'django', 'react'],
            'Data Analyst': ['data analysis', 'excel', 'power bi', 'tableau', 'sql', 'analytics'],
            'Data Scientist': ['machine learning', 'data science', 'python', 'tensorflow', 'pytorch'],
            'DevOps Engineer': ['devops', 'docker', 'kubernetes', 'jenkins', 'aws', 'azure', 'ci/cd'],
            'Mobile Developer': ['android', 'ios', 'react native', 'flutter', 'swift', 'kotlin'],
            'UI/UX Designer': ['figma', 'adobe xd', 'sketch', 'ui', 'ux', 'design'],
            'QA Engineer': ['testing', 'selenium', 'automation', 'qa', 'quality assurance'],
            'HR Professional': ['hr', 'human resource', 'recruitment', 'talent acquisition'],
        }
