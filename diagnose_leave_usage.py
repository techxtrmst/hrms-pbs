...
for emp in employees[:10]:  # Check first 10 employees
    ...
    try:
        ...
        approved_cl_requests = LeaveRequest.objects.filter(employee=emp, status="APPROVED", leave_type="CL")
        approved_cl = sum(req.total_days for req in approved_cl_requests)

        approved_sl_requests = LeaveRequest.objects.filter(employee=emp, status="APPROVED", leave_type="SL")
        ...
        if recent_leaves:
            ...
            for leave in recent_leaves:
                print(
                    f"   {leave.get_leave_type_display()}: {leave.total_days} days ({leave.start_date} to {leave.end_date})"
                )
