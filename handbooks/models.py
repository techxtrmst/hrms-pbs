...
class HandbookSection(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name="handbook_sections")
...
class Handbook(models.Model):
    ...
    # Metadata
    version = models.CharField(max_length=50, default="1.0", help_text="Version number of this handbook")
    is_published = models.BooleanField(default=False, help_text="Only published handbooks are visible to employees")
    effective_date = models.DateField(default=timezone.now, help_text="Date when this handbook becomes effective")
...
class HandbookAcknowledgment(models.Model):
    ...
    handbook = models.ForeignKey(Handbook, on_delete=models.CASCADE, related_name="acknowledgments")
    ...
    def __str__(self):
        ...
        return f"{self.employee.user.get_full_name()} - {self.handbook.title} ({status})"
...
class HandbookAttachment(models.Model):
    ...
    handbook = models.ForeignKey(Handbook, on_delete=models.CASCADE, related_name="attachments")
    title = models.CharField(max_length=200)
