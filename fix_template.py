...
{% block content %}
    <div id="content-main">
        ...
        <form id="{{ opts.model_name }}_form" method="post"
            ...
            {% endif %}
            {% if adminform.model_admin.conditional_fields %}
                x-data='{{ adminform|changeform_data }}'
            {% endif %}
            x-bind:action="activeTab && activeTab !== 'general' ? window.location.pathname + window.location.search + '#' + activeTab : window.location.pathname + window.location.search"
            {% if adminform.model_admin.warn_unsaved_form %}
                class="warn-unsaved-form"
            {% endif %}
            novalidate>
            {% csrf_token %}
            ...
            <div class="flex flex-col gap-6">
                ...
                <!-- FIX: prepopulated_fields_js causes 'super' object has no attribute 'dicts' error in Python 3.14 / Django Context -->
