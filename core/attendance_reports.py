...
def attendance_late_early_report(request):
    ...
    late_offenders = late_arrivals.values(
        'employee__id',
        ...
        late_count=Count('id'),
        total_late_minutes=Sum('late_by_minutes')
    ).order_by('-late_count')[:10]
    
    early_offenders = early_departures.values(
        'employee__id',
        'employee__user__first_name',
        'employee__user__last_name',
        'employee__badge_id'
    ).annotate(
        early_count=Count('id'),
        total_early_minutes=Sum('early_departure_minutes')
