...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if "5.7.139" in str(e):
                self.stdout.write(self.style.WARNING("   -> This usually means MFA is blocking you. You NEED an App Password."))
            if "5.7.57" in str(e):
                 self.stdout.write(self.style.WARNING("   -> This means 'Authenticated SMTP' is disabled in Admin Center."))

        if "SUCCESS" not in locals().get('status_msg', ''):
             self.stdout.write(self.style.SUCCESS("✅ Configuration Test Complete. If Step 1 passed, your email system is ready."))
