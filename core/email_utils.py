...
def send_anniversary_email(employee, years):
    ...
    try:
        ...
        logger.info(f"Anniversary email sent to {employee.user.get_full_name()} ({employee.user.email}) from {from_email} - {years} years")
        return True
...
def send_birthday_announcement(employee, company_employees):
    ...
    try:
        ...
        recipient_list = [
            emp.user.email for emp in company_employees 
        ...
        logger.info(f"Birthday announcement sent to {len(recipient_list)} employees for {employee.user.get_full_name()} from {from_email}")
...
def send_anniversary_announcement(employee, years, company_employees):
    ...
    try:
        ...
        logger.info(f"Anniversary announcement sent to {len(recipient_list)} employees for {employee.user.get_full_name()} from {from_email} - {years} years")
        return len(recipient_list)
...
def send_probation_completion_email(employee):
    ...
    try:
        ...
        logger.info(f"Probation completion email sent to {employee.user.get_full_name()} ({employee.user.email}) from {from_email}")
        return True
...
def send_regularization_request_notification(regularization_request):
    ...
    try:
        ...
        context = {
            ...
            'check_in': regularization_request.check_in.strftime('%I:%M %p') if regularization_request.check_in else 'Not specified',
            'check_out': regularization_request.check_out.strftime('%I:%M %p') if regularization_request.check_out else 'Not specified',
