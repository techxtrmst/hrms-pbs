...
def manager_dashboard(request):
    ...
    announcements = Announcement.objects.filter(
        ...
        Q(location__isnull=True) | Q(location=manager_profile.location)
    ...
    # Birthdays
    birthdays = company_employees.filter(
...
def admin_dashboard(request):
    ...
    upcoming_holidays_qs = Holiday.objects.filter(
        company=request.user.company,
        date__gte=today,
...
def personal_home(request):
    ...
    if hasattr(request.user, 'employee_profile'):
        ...
        if employee.assigned_shift:
            ...
            if total_duration <= 0: total_duration += 24 * 60 # Overnight shift
            ...
            if first_break:
                ...
                percent = (offset / total_duration) * 100
                timeline_items.append({'type': 'break', 'time': first_break.start_time, 'label': 'Break', 'percent': percent})
            else:
                 # Default middle
                 timeline_items.append({'type': 'break', 'time': '13:00', 'label': 'Lunch', 'percent': 50})
...
def employee_holidays(request):
    ...
    holidays = Holiday.objects.filter(
        ...
        year=year_filter,
        is_active=True
    ).filter(query_filter).order_by('date')

    

    # Get available years

    years = Holiday.objects.filter(
    ...
    return render(request, 'core/employee_holidays.html', {
...
def attendance_report(request):
    ...
    if request.user.role == User.Role.MANAGER:
        try:
             employees = Employee.objects.filter(manager=request.user.employee_profile).select_related('user', 'manager')
        except:
             employees = Employee.objects.none()
    else:
...
def download_attendance(request):
    ...
    for emp in employees:
        ...
        ws.cell(row=row_num, column=col_idx, value=total_days); col_idx+=1  # Total Days
        ...
        ws.cell(row=row_num, column=col_idx, value=stats['late_arrival']); col_idx+=1  # Late Arrival
        ...
        ws.cell(row=row_num, column=col_idx, value=stats['leave']); col_idx+=1  # Paid Leave Taken (Assuming all 'LEAVE' are paid for now)
        ...
        ws.cell(row=row_num, column=col_idx, value=stats['absent']); col_idx+=1  # Unpaid Leave Taken (Assuming 'ABSENT' is unpaid)

        ws.cell(row=row_num, column=col_idx, value=0); col_idx+=1  # Pending Unpaid

        ws.cell(row=row_num, column=col_idx, value=stats['absent']); col_idx+=1  # Total Unpaid
...
def leave_history(request):
    ...
    if request.user.role == User.Role.MANAGER:
        ...
        ).select_related('employee__user', 'employee__manager', 'employee__leave_balance', 'approved_by')
...
def holidays(request):
                    ...
                    for row in ws.iter_rows(min_row=2, values_only=True):  # Skip header
                        ...
                        if row[0]:  # Check if name exists
                            ...
                            try:
                                ...
                                holiday_date = row[1] if isinstance(row[1], date) else datetime.strptime(str(row[1]), '%Y-%m-%d').date()
                                ...
                                Holiday.objects.create(
                                    ...
                                    location=Location.objects.get(name__iexact=row[2]) if row[2] else Location.objects.get(name='Global'),
    ...
    years = Holiday.objects.filter(company=request.user.company).values_list('year', flat=True).distinct().order_by('-year')
    ...
    return render(request, 'core/holidays.html', {
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.error(request, "Failed to send OTP email. Please check server logs.")
